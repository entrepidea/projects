...
static std::string s_recv(zmq::socket_t & socket, int flags_ = 0) {
    zmq::message_t message;
    if (socket.recv(&message,flags_)){
...
static bool
s_send (zmq::socket_t & socket, const std::string & string, int flags_ = 0) {

    zmq::message_t message(string.size());
    memcpy (message.data(), string.data(), string.size());

    bool rc = socket.send (message, flags_);
    return (rc);
}
