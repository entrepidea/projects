  ...
  public static void main(String[] args) {
        ...
        Thread worker = new Thread() {
          public void run() {
            ...
            // might be a DB query, remote method invocation, etc.
        	  System.out.println("Is a Event Dispatch Thread? "+SwingUtilities.isEventDispatchThread());
        	  try {
        		  Thread.sleep(5000);
    ...
    bad2.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent ev) {
        ...
        // The wrong way to use invokeLater(). The runnable() shouldn't starve the dispatch thread.
        SwingUtilities.invokeLater(new Runnable() {
          public void run() {
            try {
              Thread.sleep(5000); // Dispatch thread is starving!
